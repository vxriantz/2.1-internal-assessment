"""This program displays a BOT that takes customer's orders for a gift hamper store."""
# Pizza BOT
...
# used to display product menus in a structured table format
import pandas as pd
...
# used to generate random numbers for BOT names
...
bot_names = ["Anthony", "Clint", "Gamora",
             "Howard", "Wanda", "Clara",
             "James", "Steve", "Natasha", "Sam"]
...
hamper_prices = [49.99, 39.99, 89.99, 59.99,
                 44.99, 59.99, 49.99,
...
def integer_validation(low, high, question):
    """Validates integer input for low, high, and question variables."""
...
# takes question as parameters
# removes blank spaces before checking
# input must be string
# while loop until correct input is received then returns input to original function 
# value error results in error message and new input request
def validate_alpha(question):
    """Validates alphabetical input for question variable."""
...
# takes integers 0 to 9 as randint parameters
    # prints welcome message
def welcome():
    """Prints welcome message with a random BOT name."""
...
def pickup_delivery():
    """Determines and processes the order fulfillment process."""
...
# collect click and collect data function
    ...
    # while loop until correct input is received then returns input to original function
def click_collect():
    """Ensures valid customer information input, validates and appends to dictionary."""
...
# collect delivery data
    ...
    # while loop until correct input is received then returns input to original function
def delivery_info():
    """Collects & validates delivery information & appends to dictionary."""
...
# formats prices as currency
def premade_hamper_menu():
    """Creates and formats premade hamper menu."""
...
def custom_hamper_menu():
    """Creates and formats custom hamper menu."""
    # create menu dictionary
    menu_dict = {}
...
def hamper_order():
    """
    Customer's hamper order = num of hampers,
    which hampers, prints menu, validates info before
    appending to dictionary.
    """
    num_hampers = 0  # adjusts user's input to match python's zero-based indexing
    while True:  # sets up while loop
        try:
            ...
            if num_hampers >=1 and num_hampers <= 5:  # adds boundaries to input request
                ...
                print()
                # if input is out of given boundaries, prints error messageS
                print(Fore.RED + "Your order must be between 1 and 5 hampers")
                print()
        except ValueError:
            print()
            # if input is not valid, prints error message
    ...
    # choose hampers from the menu
        # while loop until correct input is received then returns input to original function
        # item in range takes num_hampers as parameters
    ...
    for item in range(num_hampers):
        while num_hampers > 0:  # sets up while loop
            ...
            # adjusts user's input to match python's zero-based indexing
            hampers_ordered = hampers_ordered-1
            # adds name of selected hampers to the order list
            order_list.append(hamper_names[hampers_ordered])
            # add price of selected hampers to the order list
            order_cost.append(hamper_prices[hampers_ordered])
            # prints name and price of selected hampers and formats to 2dp
            print("{} ${:.2f}".format(hamper_names[hampers_ordered], hamper_prices[hampers_ordered]))
            # decreases the number of hampers the user still needs to order when ordering
...
# function displays customer order
    ...
    # accounting for differences with click and collect or delivery data
    # takes del-pick as parameter
    # returns nothing
def print_order(del_pick):
    """Prints the order receipt with customer details, order, and order cost."""
    print()
...
def continue_cancel():
    """
    Asks the customer if they would like
    to continue and purchase their order or not.
    """
...
# exit program or start a new order
    # uses integer_validation function to validate input
def new_exit():
    """Asks customer for confirmation to start a new order or exit."""
    del_pick = ""  # resets del_pick variable
...
def main():
    """Formats the program in order to be run."""
    welcome()
